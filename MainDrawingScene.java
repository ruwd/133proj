...
//					//As we are messing with opengl here, we make sure it happens in the rendering thread
        ...
        //extra
        PImage penIcon2 = pa.loadImage(imagesPath + "pen.png");
        final MTImageButton penButton2 = new MTImageButton(pa, penIcon2);
        frame.addChild(penButton2);
        penButton2.translate(new Vector3D(-50f, 600,0));
        penButton2.setNoStroke(true);
        penButton2.setStrokeColor(new MTColor(0,0,0));
        penButton2.addGestureListener(TapProcessor.class, new IGestureEventListener() {
			public boolean processGestureEvent(MTGestureEvent ge) {
				TapEvent te = (TapEvent)ge;
				if (te.isTapped()){
//    					//As we are messing with opengl here, we make sure it happens in the rendering thread
					pa.invokeLater(new Runnable() {
						public void run() {
							drawingScene.setRandomColor(!drawingScene.isRandomColor());
						}
					});
				}
				return true;
